...
export default {
  ...
  computed: {
    children() {
      // 举例：原本是 [N1, N2, N3]
      const children = this.data[this.childrenAttr]
    ...
    isNextToMe() {
      ...
      return (
        this.parentData === this.dragObj.parentData &&
        Math.abs(this.idx - this.dragObj.vmIdx) === 1
      )
    },
    isMeOrMyAncestor() {
      // 判断被拖动节点是否为“我”自身或“我”的祖先
      // let data = this.data
      let parent = this
      while (parent) {
  ...
  methods: {
    ...
    drop() {
      ...
      if (!this.isAllowToDrop) return
      // 无论如何都直接删除被拖动节点
      const index = this.dragObj.parentData[this.childrenAttr].indexOf(
        this.dragObj.data
      )
      ...
      if (this.data['_replaceLi_']) {
        if (this.dragObj.parentData === this.parentData) {
          const changedIdx = this.idx / 2
          if (index > changedIdx) {
            this.parentData[this.childrenAttr].splice(
              changedIdx,
              0,
              this.dragObj.data
            )
